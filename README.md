...
| 操作 | 意味 |
|---|---|
| Find my repos | ☰ 内。既知の作業フォルダを再スキャンして、まだ無いリポを足す |
| Open folder | ☰ 内。1リポを開く。Git でない親フォルダなら配下のリポをまとめて登録 |
| GitHub | ☰ 内。`gh` でログイン。GitHub 上の自分のリポを一覧し、未 clone なら Add、今のリポは Fetch |
| Repo ドロップダウン | この PC に実在する登録済みリポを切替 |
| Find | 枝 / tag / 件名 / ハッシュ。Enter でジャンプ。枝の **Only** でその祖先だけ表示（Ctrl+F） |
| All branches | フィルタ解除 |
| 行クリック | 件名 / 本文 / 親 / 参照 / 変更ファイル（パスのみ） |
